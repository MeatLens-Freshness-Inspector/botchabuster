...
const jsonBody = (defaultValue = "{}"): ApiDocsBody => ({
  mode: "json",
...
const authOperations: ApiDocsOperation[] = [
  operation("auth-sign-in", "auth", "POST", "/auth/sign-in", "Sign in with email and password", "Public", { body: jsonBody('{"email":"","password":""}') }),
  operation("auth-sign-up", "auth", "POST", "/auth/sign-up", "Create a user account", "Public", { body: jsonBody('{"email":"","password":"","full_name":""}') }),
  ...
  operation("auth-reset-password", "auth", "POST", "/auth/reset-password", "Send a password reset email", "Public", { body: jsonBody('{"email":""}') }),
  ...
  operation("auth-update-password", "auth", "PATCH", "/auth/users/{id}/password", "Update a user password", "Self or admin", { parameters: [pathParameter("id", "Target user id")], body: jsonBody('{"password":""}') }),
  operation("auth-recovery-password", "auth", "POST", "/auth/recovery/password", "Update a password with a recovery token", "Public", { body: jsonBody() }),
];
...
const accessCodeOperations: ApiDocsOperation[] = [
  ...
  operation("access-codes-create", "access-codes", "POST", "/access-codes", "Create an access code", "Admin", { body: jsonBody() }),
...
const inspectionOperations: ApiDocsOperation[] = [
  ...
  operation("inspections-create", "inspections", "POST", "/inspections", "Create an inspection record", "Authenticated", { body: jsonBody() }),
...
const profileOperations: ApiDocsOperation[] = [
  ...
  operation("profiles-admin-create", "profiles", "POST", "/profiles/admin/users", "Create a user as an administrator", "Admin", { body: jsonBody() }),
  operation("profiles-admin-update", "profiles", "PUT", "/profiles/admin/users/{id}", "Update a user as an administrator", "Admin", { parameters: [pathParameter("id", "Target user id")], body: jsonBody() }),
  ...
  operation("profiles-update", "profiles", "PUT", "/profiles/{id}", "Update a user profile", "Self or admin", { parameters: [pathParameter("id", "Target user id")], body: jsonBody() }),
...
const marketLocationOperations: ApiDocsOperation[] = [
  ...
  operation("market-locations-create", "market-locations", "POST", "/market-locations", "Create a market location", "Admin", { body: jsonBody() }),
...
const auditLogOperations: ApiDocsOperation[] = [
  ...
  operation("audit-logs-create", "audit-logs", "POST", "/audit-logs", "Create an audit log event", "Admin", { body: jsonBody() }),
];

const developerOptionOperations: ApiDocsOperation[] = [
  operation("developer-options-unlock", "developer-options", "POST", "/developer-options/unlock", "Unlock developer options", "Public", { body: jsonBody('{"password":""}') }),
  operation("developer-options-verify", "developer-options", "POST", "/developer-options/verify", "Verify a developer options token", "Public", { body: jsonBody('{"token":""}') }),
];
...
const developerDashboardOperations: ApiDocsOperation[] = [
  ...
  operation("developer-dashboard-datasets-export", "developer-dashboard", "POST", "/developer-dashboard/datasets/export", "Export filtered developer data", "Developer", { body: jsonBody() , responseKind: "blob", responseContentType: "application/zip" }),
...
const userChatOperations: ApiDocsOperation[] = [
  ...
  operation("user-chat-send", "user-chat", "POST", "/user-chat/messages", "Send a user chat message", "Authenticated", { body: jsonBody() }),
];
