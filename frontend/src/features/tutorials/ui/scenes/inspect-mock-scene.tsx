      ...
      <div className="flex flex-col gap-3 p-3">
        ...
        <section className="rounded-2xl border border-border/70 bg-card/90 p-3">
          ...
          <MockHotspot active={isMarket} onAdvance={onAdvance} label={isMarket ? "Choose market" : "Tap here"} ariaLabel={step.hotspotLabel}>
            <div className="rounded-xl border border-input bg-background/60 p-2"><p className="text-[8px] uppercase tracking-widest text-muted-foreground">Location Selection</p><div data-tutorial-market-selector role="combobox" aria-label="Select market location" className="mt-1 flex h-8 items-center justify-between rounded-lg border border-input px-2 text-[9px]"><span className="flex items-center gap-1"><MapPin className="h-3 w-3 text-muted-foreground" />{fixture.locationLabel}</span><ChevronDown className="h-3 w-3 text-muted-foreground" /></div><p className="mt-1 text-[8px] text-muted-foreground">Manual location is saved; {fixture.gpsStatus.toLowerCase()}.</p><p className="mt-1 text-[8px] font-medium text-foreground">Saved to report: {fixture.locationLabel}</p></div>
          </MockHotspot>
