...
import { type AuthUser } from "./SupabaseAuthOperations";
...
export interface PasskeyDatabase {
...
export class PasskeyService {
  ...
  constructor(private readonly database: PasskeyDatabase | null = null) {}
  ...
  async verifyRegistration(input: {
    ...
    const { error } = await this.getDatabase().from("passkey_credentials").upsert(row, {
  ...
  async verifyAuthentication(input: {
    ...
    const { data: authUser, error: authUserError } = await this.getDatabase().auth.admin.getUserById(storedPasskey.user_id);
    ...
    const { error: updateError } = await this.getDatabase().from("passkey_credentials")
      .update({
    ...
    return {
      user,
      session: getAppSessionService().createSession(user),
    };
  }
  ...
  async deletePasskey(userId: string, credentialId: string): Promise<void> {
    const { error } = await this.getDatabase().from("passkey_credentials")
      .delete()
      .eq("user_id", userId)
  ...
  private async listPasskeyRows(userId: string): Promise<PasskeyRow[]> {
    const { data, error } = await this.getDatabase().from("passkey_credentials")
      .select("*")
      .eq("user_id", userId)
  ...
  private getDatabase(): PasskeyDatabase {
    if (!this.database) throw new Error("Passkey database is not configured");
